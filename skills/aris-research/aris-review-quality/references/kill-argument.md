---
name: kill-argument
description: "Two-thread adversarial review: a fresh reviewer constructs the strongest 200-word rejection memo, then a second fresh reviewer defends the paper point-by-point and surfaces still-unresolved critical issues. Use when user says \"
category: research
---
...
## How This Differs From Other Review skill_view / skill_manages

| skill_view / skill_manage | What it asks the reviewer | Output |
|-------|---------------------------|--------|
...
- **CONTEXT_POLICY** = `fresh` (REVIEWER_BIAS_GUARD).  Each thread is a fresh `delegate_task (reviewer subagent)` call.  **Never** use `delegate_task (reviewer subagent)-reply`.  No prior review summary, fix list, or executor explanation enters either prompt.
...
Invoke `delegate_task (reviewer subagent)` (NOT `codex-reply`) with the following prompt structure:

```
delegate_task (reviewer subagent):
  model: gpt-5.5
...
Invoke a second `delegate_task (reviewer subagent)` call (still NOT `codex-reply` — Thread 2 is independent of Thread 1's codex history):
...
| `ERROR` | `codex_api_error` | `delegate_task (reviewer subagent)` call failed |
...
- **Fresh thread per call.**  Both Attack and Adjudication use `delegate_task (reviewer subagent)`, never `codex-reply`.  Thread 1 and Thread 2 must not share codex context.
...
After each `delegate_task (reviewer subagent)` reviewer call, save the trace following `shared-references/review-tracing.md`.  Use `tools/save_trace.sh` or write files directly to `.aris/traces/kill-argument/<date>_run<NN>/`.  Both threads' raw responses should be preserved.
...
result = terminal("python3 ~/.hermes/skills/aris-research/aris-review-quality/scripts/verify_paper_audits.py <paper_dir> --assurance submission")
```

### Output Format
This gate emits JSON verdicts to `AUDIT_KILL_ARGUMENT.json`:
```json
{
  "status": "COMPLETE",
  "verdicts": [
    {"target": "claim_1", "verdict": "PASS", "evidence": "file:line"},
    {"target": "claim_2", "verdict": "FIX", "suggestion": "..."}
  ]
}
```

### Integration with Quality Gate Runner
```
