...
## When to Use This skill_view / skill_manage
...
## What This skill_view / skill_manage Catches
...
For each **cited** bib entry — i.e., each key in `cited_keys` with at least one extracted citation context — invoke `delegate_task (reviewer subagent)` (NOT `codex-reply` — fresh thread per entry, or batch with explicit per-entry isolation). Do **not** send entries in `bib_keys \ cited_keys` to the reviewer; those are detect-only and surface only when `--uncited` is explicitly enabled (see "Uncited Entry Detection" below).
...
## Comparison with Other Audit skill_view / skill_manages

| skill_view / skill_manage | What it audits | What it catches |
|-------|---------------|-----------------|
...
After each `delegate_task (reviewer subagent)` reviewer call, save the trace following `shared-references/review-tracing.md`. Use `tools/save_trace.sh` or write files directly to `.aris/traces/citation-audit/<date>_run<NN>/`. Respect the `--- trace:` parameter (default: `full`).
...
Every invocation uses a fresh `delegate_task (reviewer subagent)` thread. Never
...
### Output Format
This gate emits JSON verdicts to `AUDIT_CITATION_AUDIT.json`:
```json
{
  "status": "COMPLETE",
  "verdicts": [
