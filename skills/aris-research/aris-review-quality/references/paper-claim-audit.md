...
description: "Zero-context verification that every number, comparison, and scope claim in the paper matches raw result files. Uses a fresh cross-model reviewer with NO prior context to prevent confirmation bias. Use when user says \"
category: research
---
...
## How This Differs From Other Audit skill_view / skill_manages

| skill_view / skill_manage | Question it answers |
|-------|-------------------|
...
**CRITICAL: Use `delegate_task (reviewer subagent)` (new thread), NEVER `delegate_task (reviewer subagent)-reply`.** Every run must be a fresh context.
...
## Integration with Other skill_view / skill_manages
...
### Output Format
This gate emits JSON verdicts to `AUDIT_PAPER_CLAIM_AUDIT.json`:
```json
{
  "status": "COMPLETE",
  "verdicts": [
