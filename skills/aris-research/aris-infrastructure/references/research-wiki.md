---
name: research-wiki
description: "Persistent research knowledge base that accumulates papers, ideas, experiments, claims, and their relationships across the entire research lifecycle. Inspired by Karpathy's LLM Wiki pattern. Use when user says \"
category: research
---
...
canonical helper, `tools/research_wiki.py`. skill_view / skill_manages that touch the wiki
...
### Cross-Skill Query
```python
# From any ARIS skill, query the wiki
from hermes_tools import terminal
result = terminal("python3 ~/.hermes/skills/aris-research/aris-infrastructure/scripts/research_wiki.py query ~/.hermes/aris-wiki 'federated learning' --max-chars 4000")
```

### Session Persistence
The wiki persists across Hermes sessions at `~/.hermes/aris-wiki/`.
Use `memory` tool to store wiki state summaries for quick recovery.

### Multi-Language Support
- `— lang: th` for Thai output
- `— lang: en` for English output (default)
- `— lang: zh` for Chinese output
