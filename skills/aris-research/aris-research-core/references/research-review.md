...
- **Codex MCP Server** configured in Claude Code:
  ...
  ```
- This gives Claude Code access to `delegate_task (reviewer subagent)` and `delegate_task (reviewer subagent)-reply` tools
...
Use `delegate_task (reviewer subagent)-reply` with the returned `threadId` to continue the conversation:
