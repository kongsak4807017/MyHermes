...
## What This skill_view / skill_manage Is — and Is NOT
...
- **CONTEXT_POLICY = `fresh`** — Each per-page review uses a **fresh** Codex thread (`delegate_task (reviewer subagent)`, never `codex-reply`). See `../shared-references/reviewer-independence.md`. This prevents the reviewer from anchoring on prior fixes.
...
- **Codex MCP**: `delegate_task (reviewer subagent)` must be available (the user must be signed in to Codex MCP). The skill aborts at Phase 0 if Codex MCP cannot be reached.
...
## Prior skill_view / skill_manage Relationship
...
- Uses the same `delegate_task (reviewer subagent)` MCP infrastructure as
