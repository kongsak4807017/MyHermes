...
- **OPTIONAL_TEXT_CRITIC = `delegate_task (reviewer subagent)`** — Optional text-only second opinion for layout/style checks
...
If `delegate_task (reviewer subagent)` is available, you may ask it for a short second-opinion
...
If `delegate_task (reviewer subagent)` is available, you may ask it for a short text-only
...
| Stage | delegate_task / Tool | Purpose |
|-------|--------------|---------|
