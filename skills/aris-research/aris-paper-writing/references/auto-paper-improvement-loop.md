...
description: "Autonomously improve a generated paper via GPT-5.4 xhigh review → implement fixes → recompile, for 2 rounds. Use when user says \"
category: research
---
...
Rules:
- Every round starts with `delegate_task (reviewer subagent)`, not `delegate_task (reviewer subagent)-reply`.
...
If `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `delegate_task (reviewer subagent)` thread for Round 2. Do not reuse the Round 1 threadId for prompting. Save the returned threadId only for recovery bookkeeping.
...
If `REVIEWER_BIAS_GUARD = false` (legacy debugging only), use `delegate_task (reviewer subagent)-reply` with the saved threadId; this is **not** the recommended path.
...
- **Large file handling**: If the Write tool fails due to file size, immediately retry using terminal (`cat << 'EOF' > file`) to write in chunks. Do NOT ask the user for permission — just do it silently.
...
- **Reviewer independence (Round 2+)**: when `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `delegate_task (reviewer subagent)` thread for every review round; never use `delegate_task (reviewer subagent)-reply` and never include "since last round" / fix summaries in the prompt. See the Reviewer Independence Protocol section above.
...
After each `delegate_task (reviewer subagent)` or `delegate_task (reviewer subagent)-reply` reviewer call, save the trace following `shared-references/review-tracing.md`. Use `tools/save_trace.sh` or write files directly to `.aris/traces/<skill>/<date>_run<NN>/`. Respect the `--- trace:` parameter (default: `full`).
