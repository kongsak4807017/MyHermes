...
description: "Draft patent claims for an invention. Use when user says \"
category: research
---
...
# Claims Drafting: The Core Patent skill_view / skill_manage
...
Call `REVIEWER_MODEL` via `delegate_task (reviewer subagent)` with xhigh reasoning:
...
4. Re-submit to examiner for round 2 (use `delegate_task (reviewer subagent)` with threadId)
...
- If `delegate_task (reviewer subagent)` is not available, skip cross-model examiner review and note it in the output.
