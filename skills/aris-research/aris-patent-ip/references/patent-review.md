---
name: patent-review
description: "Get an external patent examiner review of a patent application. Use when user says \"
category: research
---
...
Send to `REVIEWER_MODEL` via `delegate_task (reviewer subagent)` with xhigh reasoning:
...
Use `delegate_task (reviewer subagent)` with the threadId from Round 1:
