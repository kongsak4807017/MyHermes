...
description: "Full patent drafting pipeline from invention description to jurisdiction-formatted filing documents. Supports CN (CNIPA), US (USPTO), EP (EPO). Supports invention patents and utility models. Use when user says \"
category: research
---
...
- Large file handling: if a Write operation fails, retry with terminal `cat <<'EOF'` heredoc.
...
- If `delegate_task (reviewer subagent)` is not available (no OpenAI API key), skip external cross-model review and note it in the output. The pipeline must not fail due to missing reviewer access.
